...
export class GenericService<T> {
  ...
  create(itemData: any): Observable<T> {
    return this.http
      .post<T>(this.apiUrl, itemData, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError))
  }

  getAll(): Observable<T[]> {
    return this.http
      .get<T[]>(this.apiUrl, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError))
  }

  getById(id: string): Observable<T> {
    return this.http
      .get<T>(`${this.apiUrl}/${id}`, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError))
  }

  update(id: string, itemData: any): Observable<T> {
    return this.http
      .put<T>(`${this.apiUrl}/${id}`, itemData, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError))
  }
